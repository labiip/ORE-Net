...
def test_one_epoch(net, test_loader):
    ...
    with torch.no_grad():
        ...
        recalls_tgt = []
        f1s_tgt = []
        for src, target, rotation, translation, euler, gt_mask_src, gt_mask_tgt, _ in tqdm(test_loader):
...
def train_one_epoch(net, opt, train_loader):
    ...
    for src, target, rotation, translation, euler, gt_mask_src, gt_mask_tgt, _ in tqdm(train_loader):
...
if __name__ == '__main__':
    ...
    train_loader = DataLoader(
        dataset=Tooth(subset='train', partial_overlap=partial_overlap),
    ...
    test_loader = DataLoader(
        dataset=Tooth(subset='test', partial_overlap=partial_overlap),
        batch_size=batch_size,
        shuffle=False,
        num_workers=16,
        pin_memory=False,
    ...
    net = OverlapNet(src_overlap=3000, tgt_overlap=3000, all_points=5000)
    ...
    if RESUME:
        path_checkpoint = "./checkpoint/ckpt_best400.80.8.pth"
        checkpoint = torch.load(path_checkpoint, map_location=lambda storage, loc: storage.cuda(gpu_id)) 

...
class OverlapNet(nn.Module):
    def __init__(self, src_overlap=512, tgt_overlap=512, n_emb_dims=512, all_points=1024):
        super(OverlapNet, self).__init__()
        self.emb_dims = n_emb_dims
        self.all_points = all_points
        self.src_overlap = src_overlap
        self.tgt_overlap = tgt_overlap
        self.emb_dims1 = int(self.emb_dims / 2)
        self.emb_nn1 = DGCNN(self.emb_dims1,k=20)
        ...
        self.mask_src_nn = nn.Sequential(
            nn.Conv1d(self.all_points, 1024, 1), nn.BatchNorm1d(1024), nn.LeakyReLU(negative_slope=0.01),
    ...
    def forward(self, *input):
        ...
        values, indices = torch.topk(mask_src_score, k=self.src_overlap, dim=1)
        mask_src_idx.scatter_(1, indices, 1)  # (dim, 索引, 根据索引赋的值)

        mask_tgt_idx = torch.zeros(mask_tgt_score.shape).cuda()
        values, indices = torch.topk(mask_tgt_score, k=self.tgt_overlap, dim=1)

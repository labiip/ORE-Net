...
from areconv.modules.transformer.lrpe_transformer import LRPETransformerLayer
from areconv.modules.transformer.pe_transformer import PETransformerLayer
from areconv.modules.transformer.rpe_transformer import MSMLTransformerLayer
from areconv.modules.transformer.vanilla_transformer import TransformerLayer
...
class MSIITransformer(nn.Module):
    def __init__(
        self,
        blocks,
        d_model,
        ...
        super(MSIITransformer, self).__init__()
        self.blocks = blocks
        layers = []
        num = 0
        for block in self.blocks:
            _check_block_type(block)
            if block == 'self':
                layers.append(MSMLTransformerLayer(d_model, num_heads, num, dropout=dropout, activation_fn=activation_fn))
                num+=1
            else:
                layers.append(TransformerLayer(d_model, num_heads, dropout=dropout, activation_fn=activation_fn))
        self.layers = nn.ModuleList(layers)
    ...
    def forward(self, feats0, feats1, embeddings0, embeddings1, masks0=None, masks1=None):
        ...
        for i, block in enumerate(self.blocks):
            if block == 'self':
                feats0, scores0 = self.layers[i](feats0, embeddings0, memory_masks=masks0)
                feats1, scores1 = self.layers[i](feats1, embeddings1, memory_masks=masks1)
            else:
                if self.parallel:
